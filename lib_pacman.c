...
void mostraLabirinto(struct labirinto labirinto){
    ...
    for (linha = 0; linha < labirinto.qtdLin; linha++){
        for (coluna = 0; coluna < labirinto.qtdCol; coluna++){
            if ( labirinto.matriz[linha][coluna] == 0 ){
                atributo = COLOR_PAIR(1);
                attron(atributo);
                mvprintw(linha + 1, coluna + 1, VAZIO);
                attroff(atributo);
            }
            else if ( labirinto.matriz[linha][coluna] == 1 ){
                atributo = COLOR_PAIR(1);
                attron(atributo);
                mvprintw(linha + 1, coluna + 1, PASTILHA_NORMAL);
                attroff(atributo);
            }
            else if ( labirinto.matriz[linha][coluna] == 2 ){
                atributo = COLOR_PAIR(1);
                attron(atributo);
                mvprintw(linha + 1, coluna + 1, PASTILHA_ESPECIAL);
                attroff(atributo);
            }
            else {
                atributo = A_REVERSE | COLOR_PAIR(1); 
                attron(atributo);
                mvprintw(linha + 1, coluna + 1, BARREIRA);
                attroff(atributo);
            }
        }
    }
